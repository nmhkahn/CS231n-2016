...
class ConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=(32), filter_size=7,
    ...
    # fc-10 layer
    W6 = np.random.randn(num_filters[3], num_classes) / np.sqrt(num_filters[3] / 2)
    b6 = np.zeros(num_classes)
    
    self.params["W1"] = W1; self.params["W2"] = W2; self.params["W3"] = W3
    self.params["W4"] = W4; self.params["W6"] = W6
    self.params["b1"] = b1; self.params["b2"] = b2; self.params["b3"] = b3
    self.params["b4"] = b4; self.params["b6"] = b6
  ...
  def loss(self, X, y=None):
    ...
    fc6, fc6_cache  = affine_forward(ap5, W6, b6)
    ...
    loss += 0.5 * self.reg*(np.sum(W1**2)+np.sum(W2**2)+np.sum(W3**2)+np.sum(W4**2)+np.sum(W6**2))
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    end = time.time()
